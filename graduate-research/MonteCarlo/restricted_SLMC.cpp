...
int make_cluster(int S[N], double J_eff, double T, double& weight, int& n_cluster, int(&i_cluster)[N][1],int& size,  mt19937 &engine){
    int in_out[N];
    int pair[N][4];
    make_pair(pair);
            ...
            if(S[pair[chosen_site][j]] == spin_cluster){
                if(in_out[pair[chosen_site][j]]==0){//まだクラスターに追加されていないものが対象
                    ...
                    if(distance<r){
                        ...
                        weight *= exp((2/T)*J_eff*S[chosen_site]*S[pair[chosen_site][j]]*-1);
                    }
                }
            }
        }
...
void restrict_SLMC(int S[N],  int pair[][4], int n_cluster, int spin_cluster, int i_cluster[N][1], double beta,double weight, mt19937 &engine){
    uniform_real_distribution<double> dist2(0, 1);
    int S_[N];
    for (int i=0; i<N; i++){
        S_[i] = S[i];
...
int main(){
    ...
    for (int seed=2022; seed<2027; seed++){
        ...
        for(int step=0; step<(100000-1)*MCS; step++){
            int n_cluster; //クラスターに属するスピンの個数
            int i_cluster[N][1]; //クラスターに属するスピンのインデックス
            int size = 0;
            double weight =1;

            int spin_cluster = make_cluster(S, J_eff, T, weight, n_cluster, i_cluster, size, engine);
            //make_cluster１回ごとにi_clusterとn_clusterの値が書き換わる//

            restrict_SLMC(S, pair, n_cluster, spin_cluster, i_cluster, beta, weight, engine);
            E = Energy(S, pair);
            M = Magnetization(S);
