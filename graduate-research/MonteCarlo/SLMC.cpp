...
int make_cluster(int S[N], double J_eff, double T, int& n_cluster, int(&i_cluster)[N][1],int& size,  mt19937 &engine){
    int in_out[N];
    int pair[N][4];
    make_pair(pair);
...
void SLMC(int S[N],  int pair[][4], int n_cluster, int spin_cluster, int i_cluster[N][1], double beta, mt19937 &engine){
    uniform_real_distribution<double> dist2(0, 1);
    int S_[N];
    for (int i=0; i<N; i++){
        S_[i] = S[i];
    ...
    //cout << accept_ratio << endl;

    if (dist2(engine) < exp(-1*beta*(del_beta-del_alpha))){
        for (int k=0; k<n_cluster; k++){
            int i = i_cluster[k][0]; //クラスターに入っているスピンのインデックスを取り出す
            S[i] *= -1; //スピンをフリップ
        } 
...
int main(){
    ...
    for (int seed=2022; seed<2027; seed++){
        ...
        ofstream file1;
        file1.open("./output/L70/seed"+to_string(seed)+"_SLMC/energy_mag_"+ to_string(seed)+"_" + to_string(T).substr(0,4) + ".dat", ios::app);
        ...
        for(int step=0; step<(50000-1); step++){
            int n_cluster; //クラスターに属するスピンの個数
            int i_cluster[N][1]; //クラスターに属するスピンのインデックス
            int size = 0;

            int spin_cluster = make_cluster(S, J_eff, T, n_cluster, i_cluster, size, engine);
            //make_cluster１回ごとにi_clusterとn_clusterの値が書き換わる//

            SLMC(S, pair, n_cluster, spin_cluster, i_cluster, beta, engine);
            E = Energy(S, pair);
            M = Magnetization(S);
