...
double get_restrict_weight(int count, double beta){
    double weight;
    weight = exp(2*count*J_eff*beta);
    return weight;   
}

int make_cluster(int S[N], double J_eff, double T, double& weight, int& distance, int& n_cluster, int(&i_cluster)[N][1],int& size,  mt19937 &engine){
    int in_out[N];
    int pair[N][4];
    make_pair(pair);
        ...
        for(int j=0; j<4; j++){
            if(S[pair[chosen_site][j]] == spin_cluster){//同符号のスピン
                if(in_out[pair[chosen_site][j]]==0){//まだクラスターに追加されていないものが対象
                    ...
                    cout << i_cluster[0][0]<< " " << pair[chosen_site][j] << " "<<  distance << endl;
                    if(r>distance){
                        ...
                        weight *= exp(-2*beta*J_eff*S[chosen_site]*S[pair[chosen_site][j]]);
...
int main(){
    ...
    for(int step=0; step<50000; step++){
        ...
        int distance = 1;
        double weight =1;//重みの初期化
        int spin_cluster  = make_cluster(S, J_eff, T, weight, distance, n_cluster, i_cluster,size,  engine);
